...
export class ItemStack {
    /**
     * @remarks
     * 此物品实例中的物品数量。有效值范围为 1-255。提供的值将被限制到该物品的最大堆叠数量。
     *
    ...
    /**
     * @remarks
     * 返回该物品是否可堆叠。如果物品的最大堆叠数量大于 1，且不包含任何自定义数据或属性，则该物品被视为可堆叠。
     *
     */
    ...
    /**
     * @remarks
     * 最大堆叠数量。该值根据物品类型不同而变化。例如，火把的最大堆叠数量为 64，而鸡蛋的最大堆叠数量为 16。
     *
     */
    readonly maxAmount: number;
    /**
     * @remarks
     * 此物品实例的给定名称。悬停在物品上时会显示名称标签。将名称标签设置为空字符串或 `undefined` 将移除名称标签。
     *
     * This property can't be edited in restricted-execution mode.
     *
     * @throws
    ...
    /**
     * @remarks
     * 此物品实例的物品类型标识符。如果未指定命名空间，则默认为 'minecraft:'。例如 'wheat' 或 'apple'。
     *
     */
    readonly typeId: string;
    /**
     * @remarks
     * 此物品实例中所有物品的总重量，加上通过 `Storage Item` 组件定义的物品容器中所有物品的重量。每个物品的重量可以通过 `Storage Weight Modifier` 组件修改。
     *
     */
     ...
     * @param amount
     * 要放入物品实例中的物品数量，范围为 1-255。提供的值将被限制到该物品的最大堆叠数量。请注意，某些物品只能有一个物品在堆叠中。
     * 默认值：1
     * 范围：[1, 255]
     * @throws
     ...
     *
     * @param componentId
     * 组件的标识符（例如 'minecraft:food'）。如果未指定命名空间前缀，则默认为 'minecraft:'。可用的组件 ID 是 {@link
     * ItemComponentTypes} 枚举中的 ID，以及通过 {@link ItemComponentRegistry} 注册的自定义组件 ID。
     * @returns
     * 如果组件存在于此物品实例上，则返回该组件；否则返回 undefined。
    ...
    /**
     * @remarks
     * 返回此物品实例当前存储的所有动态属性的总大小（以字节为单位）。这包括键和值的大小。这对于诊断性能警告信号很有用——例如，如果某个物品实例关联了数兆字节的动态属性，在各种设备上加载可能会很慢。
     *
     * @returns
     * 此物品实例当前存储的所有动态属性的总字节数。
     */
    getDynamicPropertyTotalByteCount(): number;
    /**
     * @remarks
     * 返回 ItemStack 的 lore 值——一个辅助显示字符串。
     *
     * @returns
     * 一个 lore 行的数组。如果物品没有 lore，则返回空数组。
     */
    getLore(): string[];
    /**
     * @remarks
     * 返回 ItemStack 的 lore 值——一个辅助显示字符串。字符串类型的 lore 行将被转换为 {@link
     * RawMessage}，并放在 {@link RawMessage.text} 下。
     *
     * @returns
     * 一个 lore 行的数组。如果物品没有 lore，则返回空数组。
     */
...
getRawLore(): RawMessage[];
    ...
    /**
     * @remarks
     * 如果此物品实例上存在指定的组件，则返回true。
     *
     * @param componentId
     * 要检索的组件标识符（例如'minecraft:food'）。若未指定命名空间前缀，则默认为'minecraft:'。
     * @returns
     * 如果组件存在则返回true，否则返回false。
     */
     ...
     * 要搜索的标签。
     * @returns
     * 如果物品实例具有该关联标签则返回true，否则返回false。
     */
    hasTag(tag: string): boolean;
    /**
     * @remarks
     * 返回此物品实例是否可与给定的`itemStack`堆叠。这是通过比较物品类型及与物品实例关联的任何自定义数据和属性来确定的。每个物品实例的数量不予考虑，但对于不可堆叠的物品，此方法始终返回false。
     *
     * @param itemStack
     * 要检查堆叠兼容性的物品实例。
     * @returns
     * 如果物品实例可与传入的itemStack堆叠则返回true；对于不可堆叠的物品则返回false。
     */
     ...
     * 物品的标识符。
     * @param states
     * 仅适用于方块。可选的用于比较的一组状态。若未指定states，则匹配检查更广泛的类型集合。
     * @returns
     * 返回一个布尔值，指示指定的物品是否匹配。
     */
    ...
    /**
     * @remarks
     * 此物品在冒险模式下可破坏的方块类型列表。方块名称显示在物品的工具提示中。将值设置为undefined会清除列表。
     *
     * This function can't be called in restricted-execution mode.
     *
    ...
    /**
     * @remarks
     * 此物品在冒险模式下可放置的方块类型列表。仅适用于方块物品。方块名称显示在物品的工具提示中。将值设置为undefined会清除列表。
     *
     * This function can't be called in restricted-execution mode.
     *
     * @param blockIdentifiers
     ...
     *
     * @param values
     * 要设置的动态属性的键值对记录。如果数据值为null，则改为移除该属性。
    ...
    /**
     * @remarks
     * 设置物品实例的 lore 值（辅助显示字符串）。若设置为空字符串或undefined，则清除 lore 列表。
     *
     * This function can't be called in restricted-execution mode.
     *
     * @param loreList
     * lore 行列表。列表中的每个元素代表一个新行。最大 lore 行数为20。每行 lore 最大长度为50个字符。
     * @throws This function can throw errors.
     *
     * {@link minecraftcommon.ArgumentOutOfBoundsError}
     *
